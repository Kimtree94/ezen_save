/*
	  ...
	  js : id명
			...
			//4. 게시물 여러개 저장할 공간 = 배열 사용 [원래는 db사용]
			//5. 글 등록이 완료 될때마다 board객체를 배열에 저장한다.
				//6.글 등록했으면 기존 내용물 지우기
			[출력]
			//1. js 시작될때/ 새로운 글이 등록 될떄마다 [게시물 출력]
			[보기]
...
function 게시물출력(){
	...
	for(let i=0 ;i<boardlist.length;i++){
		//i는 0부터 배열의 길이까지 1씩 증가             //배열내 i번째 객체내 '제목'키 호출 -> 값이 나옴
	html+= '<tr><td>'+i+'</td><td><a href="#" onclick="게시물보기('+i+')">'+boardlist[i].제목+'</a></td><td>'+boardlist[i].작성자+'</td></tr>'
											//" ": 문자 처리  '':문자처리 vs`${}`
											//js에서 html 작성시 문자처리'<태그명 onclock="함수()">' ''사용했으면 ""
	}
	//
