...
function 게임결과(){
	...
	for( let i=0 ; i<=6 ; i+=3){
		//i는0부터 6까지 3씩 증가반복[3회 : 0 3 6]
		if(게임상황[i] !=null && 게임상황[i]==게임상황[i+1] && 게임상황[i+1]==게임상황[i+2] ){
						//&& -> 이면서 면서 이고 모두 그리고
							//i가 0일때 0==1==2 //i가 3일때 3==4==5 //i가 6일때6==7==8
	...
	for(let i=0;i<=2;i+=2){
	 ...
	 if(게임상황[i]!= null&& 게임상황[i]==게임상황[i+4]&&게임상황[i+4]==게임상황[i+8]){
		승리=게임상황[i]  
		return true
	}else if(게임상황[i] != null&&게임상황[i]==게임상황[i+2]&&게임상황[i+2]==게임상황[i+4]){
		승리=게임상황[i]	
		return true	
	}
	}
	...
	/*마지막 버튼이 안눌림?컴퓨터는 결과를 내야해서????
	그럼 리턴값을 버튼이 다 눌릴때까지 못받으면 무승부로 ??
