/*
	...
	1.익명함수 : 이름 없는 함수 
		function (){ }
	2.선언적함수 : 일반적인 함수 기본함수
 		function 함수명 (){ }
 	3. 나머지매개변수 : (...매개변수){ } 매개변수가 고정적이지 않는 함수
 		function 함수명(...나머지매개변수){ }
 	4.기본매개변수 : 매개변수의 초기값 설정
 		function 함수명(매개변수=값){ }
 	5. 콜백함수 :  함수도 자료형이기 떄문에 매개변수로 전달이 가능하다.
 		function 함수명 (매개변수){
    ...
    *	미리 만들어진 콜백 사용하는 배열내 함수들
    	foreach()	: 반복문
    	map()	    :새로운 배열 생성[return 값을 새로운 배열에 저장]
    	filiter()   :새로운 배열 생성[return이 true인 경우에만 값을 새로운 배열에 저장]
	6. 화살표 함수 : function생략
		(매개변수) => 결과
	...
	/*
		//화살표 함수를 사용했을때
			//메소드 체이닝 (결과 연결 ) 사용하지 않을때
				//배열명.메소드1().메소드2() :메소드 1 결과를 가지고 메소드2 실행
	numbers 
