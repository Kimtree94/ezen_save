...
function 게임판그리기(){
	//function 함수 만들기 키워드
		//게임판그리기 라는 이름으로 함수 선언[매개변수 x 리턴값 x]
	let 게임판 ='' //여려개 버튼(tag)을 저장하는 변수
...
//2. 버튼 선택 함수
function 버튼선택 (i){
	// 사용자 선택한 버튼의 id의 html -> o 로 변경
	document.getElementById(i).innerHTML = 'ㅇ'
	//[컴퓨터] 난수 발생
		//Math         // Math : 수학 관련 메소드 제공하는 클래스
		// rnadom()		//Math.random( ) : 0~1사이의 난수를 발생	
                        // Math.random()*끝번호-1
					//(Math.random()* 끝번호+1)
