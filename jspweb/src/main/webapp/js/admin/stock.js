
/*

	- 함수
		1. 함수명()
		2. funtion(인수){ }
		3. 인수 =>{ }

	함수안에 함수 함수가 함수를 부르는것을  콜백함수라고 한다 
	- 콜백함수 : 함수실행 반환 결과를 [콜백]함수에게 인수로 전달 
		함수1 에서 실행된 반환값을 콜백함수2의 인수로 전달 
	
	1.함수1(콜백함수2(인수))
	2.함수1(인수 => funtion(인수){})
	3.함수1( 인수를 전달 => 콜백 함수2 )
	
	- 활용 
	.addEventListener ('이벤트명' , 함수호출[1~3]	)
			: 이벤트 실행 결과정보를 반환 
	.forEach ( 인수 => { 코드 } )

*/
let productlist =null;

let cselect = document.querySelector('.cselect') // 카테고리선택상자
let pselect = document.querySelector('.pselect') // 제품 선택 상자 
let rows = document.querySelectorAll('.rows')//재고 등록 td행 

// 1-1 카테고리목록에서 선택을 클릭했을때  제품출력 메소드 
cselect.addEventListener('click', e=>{ 
	let pcno=e.currentTarget.value; // 선택된 카테고리 번호값 
	getproduct(pcno); // 선택된 카테고리 번호 인수로 전달 
})

// 1-2 제품목록에서 선택을 했을때 재고 등록 html 구성
pselect.addEventListener('click', e=>{ 
	let pno=e.currentTarget.value; // 선택된 카테고리 번호값 
	productlist.forEach(p=>{ // 해당 제품의 번호와 선택된 제품의번호와 같으면 
		if(p.pno==pno){
			rows[0].innerHTML=p.pcno
			rows[1].innerHTML=p.pno
			rows[2].innerHTML=p.pname
			getstock();//제품을 클릭했을떄 제품재고 메소드 호출 
		}
	})
})


/* 1. 페이지 열리면 모든 카테고리 select로 넣어주기 */
getcategory();
function getcategory() {
	$.ajax({
		url: "/jspweb/board/pcategory",
		type: "get",
		success: function(re) {
			let json = JSON.parse(re)
			
			// 배열혹은리스트[객체].forEach(반복변수명[아무거나],인덱스,배열객체명=>{실행코드})
			//forEach(value,index,array) 순서  
			let html = '<option>카테고리 선택</option>';
			json.forEach(c => {
			 	html += `<option value=${c.pcno}>${c.pcname}</option>`
			})
			
			cselect.innerHTML =html;
		}
	})
}


/* 2. 선택된 카테고리별 제품리스트를 select 넣어주기 */

function getproduct(pcno){
	$.ajax({
		url: "/jspweb/admin/regist",
		data:{"type" :1 ,"option":"all"},
		type: "get",
		success: function(re) {
		 	let html = '';
		 	productlist=JSON.parse(re)
			productlist.forEach(p => {
				if(p.pcno==pcno){ // 해당제품의 카테고리번호와 선택된 카테고리 번호와 같으면 
			productlist = JSON.parse(re) //대입 
					
			 	html += `<option value=${p.pno}>${p.pname}</option>`
				}
			})
			pselect.innerHTML =html;	
		}
	})
}


//3. 재고 등록버튼을 눌렀을때
 function setstock(){
		
		//1. 등록할 데이터 구성한다 .
		let info={
			psize : document.querySelector('.psize').value,
			pcolor : document.querySelector('.pcolor').value,
			pstock : document.querySelector('.pstock').value,
			pno : rows[1].innerHTML
		}
	//2. 통신
	$.ajax({
		url :"/jspweb/admin/stock",
		type:"post",   // post method
		data: info,
		success:function(re){
			if(re=='true'){alert('재고등록성공');getstock();}
			else{alert('재고등록실패')}
		}
	})
}

getstock();
//4. 제품별 재고 출력 
function getstock(){
	$.ajax({
		url:"/jspweb/admin/stock",
		type:"get",
		data:{"pno":rows[1].innerHTML},
		success:function(re){
			let json = JSON.parse(re);
			let html='';
			
			for(let i=0; i<json.length; i++){
				let list = json[i]
			html +=	`<tr>`+
					`<th>${list.psize}</th>`+
					`<th>${list.pcolor}</th>`+
					`<th>${list.pstock}</th>`+
					`<tr>`
						
			}
			console.log("html.////"+html)
			document.querySelector('.stock').innerHTML=html;
		}
	})
}




















