...
/*
	JS반복문
		1. for (let indexi =0 ; 배열명.length ;index++){ }
			i : 반복변수 [ 인덱스용 ]
		2. 배열명.forEach( (value , index , array ) => { }) or (함수 (i))
				value : 배열내 데이터 반복변수 
				index : 인덱스 반복변수
				array : 배열
		3. for( value of 배열명 ){ }
...
document.querySelector('.color_select').addEventListener('change' , (e)=>{
	color = e.currentTarget.value	// 이벤트를 발생한 객체[color_select]의 담겨져있는 값 
	...
	document.querySelector('.size_select').innerHTML = shtml
})

// **사이즈 select 박스를 체인지 했을때 이벤트
document.querySelector('.size_select').addEventListener('change' , (e)=>{
...
btnlike.addEventListener('click',(e)=>{
	//1.로그인 유무 판단 [1.ajax 통신해서 세션 유무를 확인한다  2.html[jsp] 에서 가져온다 ]
...
function sethtmlprint(){
		...
		document.querySelector('.color_select').innerHTML = chtml
}
// 3. 재고 가져오는 함수 메소드 
...
function print(){
	...
	productlist.forEach( (p , i) => {
		let tsale =  psale*p.amount;	//(할인이적용된)판매가 * 수량
		let tpoint = Math.round(tsale*0.01);		//((할인이적용된)판매가*수량 )*0.01 
...
function amountup(i){
	...
	productlist[i].pcolor // 선택한 칼라
	productlist[i].psize  // 선택한 사이즈 
