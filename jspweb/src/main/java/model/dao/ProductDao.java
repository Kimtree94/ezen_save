...
public class ProductDao extends Dao {
	...
	public ArrayList<ProductDto> getProductlist(String option) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				ProductDto dto = new ProductDto(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4),
						rs.getFloat(5), rs.getByte(6), rs.getString(7), rs.getString(8), rs.getInt(9));
				list.add(dto);
			}
		} catch (Exception e) {
			System.out.println(e);
	...
	public boolean deleteproduct(int pno) {
		...
		System.out.println("제품삭제 다오 ::"+pno);
		try {
			ps = con.prepareStatement(sql);
	...
	public int setplike(int pno, int mno) {
		...
		try {
			...
			if(rs.next()) {//이미 찜하기가 되어있는 경우 // 검색 결과가 있으면 =>취소
	...
	public boolean setOrder(ArrayList<OrderDto> list) {
		...
		try {
			...
			if (rs.next()) {
				...
				ps.setInt(3, list.get(i).getOdamaount());
